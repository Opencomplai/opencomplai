...
### Scan exclusions (`.ocignore`)

`opencomplai scan` and `opencomplai check` do **not** read `.gitignore` at scan
time. Exclusion patterns and inventory limits live in a repo-root `.ocignore`
instead (gitignore-like `fnmatch` patterns; trailing `/` marks a directory).

On first scan the CLI writes a default `.ocignore` if one is missing (pass
`--no-ocignore-bootstrap` to skip that). You can seed it from `.gitignore` only
during that first-run bootstrap; later edits to `.gitignore` are not imported
again. Commit `.ocignore` so CI and local scans stay consistent.

Minimal example:

```gitignore
# .ocignore — scanner exclusions and limits
[limits]
max_files = 10000
skip_binary = true
...
Full syntax, limits, and flag reference:

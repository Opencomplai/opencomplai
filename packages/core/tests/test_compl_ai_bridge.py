"""Tests for the optional COMPL-AI benchmark bridge (opencomplai eval --suite compl-ai)."""
...
def test_eval_log_to_evaluator_result_has_no_hard_inspect_dependency():
    ...
    assert result.evaluator_id == "EVAL_COMPL_AI_TOXICITY_V1"
    assert result.outcome == EvaluatorOutcome.PASS
    assert "compl_ai_task=toxicity" in result.findings
    assert "COMPL-AI" in result.reference
...
def test_run_compl_ai_suite_raises_import_error_without_extra():
    ...
    with pytest.raises(ImportError, match="compl-ai-bridge"):
        run_compl_ai_suite(["toxicity"], model="gpt-4o-mini", api_key="fake-key")
...
def test_default_opencomplai_eval_path_is_unaffected_by_bridge_absence():
    ...
    import opencomplai_core.bridges.compl_ai  # noqa: F401
